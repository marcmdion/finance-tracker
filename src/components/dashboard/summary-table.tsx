...
export function SummaryTable({
  ...
  const columnCount = summaryData.cycles.length + 2;
    ...
    return entries.map(([category, amounts]) => {
      ...
      return (
        <TableRow key={`${type}-${category}`} className="border-border/30">
          ...
          <TableCell className="sticky right-0 z-10 bg-card/95 text-right text-sm font-medium tabular-nums backdrop-blur-sm">
            {rowTotal > 0 ? (
              <span className="whitespace-nowrap">
                ${rowTotal.toFixed(2)}
                {type === "expense" && (
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border/40 hover:bg-transparent">
              <TableHead className="sticky left-0 z-20 h-11 bg-card/95 p-0 backdrop-blur-sm">
                <div className="flex h-full items-center gap-3 px-3">
                  <button
                    type="button"
                    onClick={() => {
                      const next = nextSortState(incomeSort, "category");
                      setIncomeSort(next);
                      setExpenseSort(next);
                    }}
              ...
              {summaryData.cycles.map(([key, label]) => (
                <TableHead key={key} className="h-11 p-0">
                  <button
                    type="button"
                    onClick={() => {
                      const next = nextSortState(incomeSort, key);
                      setIncomeSort(next);
                      setExpenseSort(next);
                    }}
              ...
              <TableHead className="sticky right-0 z-20 h-11 bg-card/95 p-0 backdrop-blur-sm">
                <button
                  type="button"
                  onClick={() => {
                    const next = nextSortState(incomeSort, "total");
                    setIncomeSort(next);
                    setExpenseSort(next);
                  }}
                  className="label-caps flex h-full w-full items-center justify-end gap-1.5 px-3 hover:text-foreground"
          ...
          <TableBody>
            <TableRow className="border-border/30 bg-muted/20 hover:bg-muted/20">
              <TableCell colSpan={columnCount} className="label-caps py-3">
                Income
              </TableCell>
            </TableRow>
            {renderCategoryRows(
            ...
            <TableRow className="border-border/50 bg-muted/30 hover:bg-muted/30">
              ...
              <TableCell className="sticky right-0 z-10 bg-muted/80 metric-value-sm text-right text-emerald-600/90 dark:text-emerald-400/90">
            ...
            <TableRow className="border-border/50 bg-muted/30 hover:bg-muted/30">
              ...
              <TableCell className="sticky right-0 z-10 bg-muted/80 metric-value-sm text-right text-rose-600/90 dark:text-rose-400/90">
            ...
            <TableRow className="border-border/50 bg-muted/50 hover:bg-muted/50 dark:bg-muted/35">
              ...
              <TableCell
                className={cn(
                  "sticky right-0 z-10 bg-muted/50 metric-value-sm text-right dark:bg-muted/35",
